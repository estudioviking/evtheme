...
function html_contact_form() {
	$html = '<form id="contact-form" action="' . esc_url( get_the_permalink() ) . '" method="post">' .
				...
				'</div>' .
				'<div class="form-group">' .
					'<label for="cf-email">' . __( 'Email', 'viking-theme' ) . ' <span class="required">*</span></label>' .
				...
				'</div>' .
				'<div class="form-group">' .
					'<label for="cf-subject">' . __( 'Subject', 'viking-theme' ) . ' <span class="required">*</span></label>' .
					'<input type="text" name="cf-subject" placeholder="' . __( 'What is the subject of the message?', 'viking-theme' ) . '" value="' . ( isset( $_POST['cf-subject'] ) ? esc_attr( $_POST['cf-subject'] ) : '' ) . '" size="40" required>' .
				'</div>' .
				'<div class="form-group">' .
					'<label for="cf-message">' . __( 'Message', 'viking-theme' ) . ' <span class="required">*</span></label>' .
					'<textarea cols="45" rows="8" name="cf-message" placeholder="' . __( 'Enter your message here.', 'viking-theme' ) . '" required>' . ( isset( $_POST['cf-message'] ) ? esc_attr( $_POST['cf-message'] ) : '' ) . '</textarea>' .
				'</div>' .
				'<p class="form-submit"><input type="submit" name="cf-submitted" value="' . __( 'Send', 'viking-theme' ) . '"></p>' .
			'</form><!-- #contact-form -->';
...
function deliver_contact_form_mail() {
	...
	if ( isset( $_POST['cf-submitted'] ) ) :
		...
		$success = __( 'Thanks for contacting us, expect a response soon.', 'viking-theme' );
		$error = __( 'An unexpected error occurred. Try again later.', 'viking-theme' );
